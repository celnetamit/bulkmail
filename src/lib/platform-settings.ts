...
const DEFAULT_CAMPAIGN_SEND_CONCURRENCY = Number(process.env.CAMPAIGN_SEND_CONCURRENCY || 5);
...
export function ensurePlatformSettingsSchema() {
  ...
  executeSql(`
    CREATE TABLE IF NOT EXISTS "PlatformSettings" (
      ...
      "campaignSendConcurrency" INTEGER NOT NULL DEFAULT ${Number.isFinite(DEFAULT_CAMPAIGN_SEND_CONCURRENCY) && DEFAULT_CAMPAIGN_SEND_CONCURRENCY > 0 ? Math.min(25, Math.floor(DEFAULT_CAMPAIGN_SEND_CONCURRENCY)) : 5},
      "sendingDomain" TEXT,
  ...
  executeSql(
    ...
    `,
    [
      Number.isFinite(DEFAULT_IMAGE_UPLOAD_LIMIT_KB) && DEFAULT_IMAGE_UPLOAD_LIMIT_KB > 0
        ? Math.floor(DEFAULT_IMAGE_UPLOAD_LIMIT_KB)
        : 50,
      Number.isFinite(DEFAULT_CAMPAIGN_SEND_CONCURRENCY) && DEFAULT_CAMPAIGN_SEND_CONCURRENCY > 0
        ? Math.min(25, Math.floor(DEFAULT_CAMPAIGN_SEND_CONCURRENCY))
        : 5,
    ],
  );
...
export async function getPlatformSettings(): Promise<PlatformSettingsView> {
  ...
  const row = queryRow<PlatformSettingsRow>(
    'SELECT "id", "imageUploadLimitKb", "campaignSendConcurrency", "sendingDomain", "spfVerified", "dkimVerified", "dmarcVerified", "createdAt", "updatedAt" FROM "PlatformSettings" WHERE "id" = ? LIMIT 1',
    ['global'],
  );
  ...
  return {
    ...
    campaignSendConcurrency: row?.campaignSendConcurrency || (Number.isFinite(DEFAULT_CAMPAIGN_SEND_CONCURRENCY) && DEFAULT_CAMPAIGN_SEND_CONCURRENCY > 0 ? Math.min(25, Math.floor(DEFAULT_CAMPAIGN_SEND_CONCURRENCY)) : 5),
...
export async function savePlatformSettings(input: {
  ...
  const campaignSendConcurrency =
    Number.isFinite(input.campaignSendConcurrency) && (input.campaignSendConcurrency || 0) > 0
      ? Math.min(25, Math.floor(input.campaignSendConcurrency as number))
      : Math.min(
          25,
          Math.floor(
            current?.campaignSendConcurrency ||
              (Number.isFinite(DEFAULT_CAMPAIGN_SEND_CONCURRENCY) && DEFAULT_CAMPAIGN_SEND_CONCURRENCY > 0 ? DEFAULT_CAMPAIGN_SEND_CONCURRENCY : 5),
          ),
        );
  ...
  executeSql(
    `
      ...
      ON CONFLICT("id") DO UPDATE SET
        ...
        "campaignSendConcurrency" = excluded."campaignSendConcurrency",
