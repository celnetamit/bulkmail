...
const postgresIdentifiersToQuote = [
  ...
  'label',
  'description',
  // camelCase SQL aliases (e.g. `SELECT COUNT(*) as campaignCount`). Postgres
  // folds unquoted identifiers to lowercase, so without quoting these come back
  // as `campaigncount`/`contactscount`/... and the camelCase JS property access
  // reads `undefined`. That silently breaks dashboard/analytics numbers and, for
  // `campaignCount`, skips the "list in use" guard so a RESTRICT foreign key on
  // Campaign.listId turns a list delete into a 500. SQLite preserves alias case,
  // which is why these only fail on the Postgres (live) path.
  'campaignCount',
  'contactsCount',
  'campaignsCount',
  'listsCount',
